...
def do_grabcut(img0, preview=-1, nb_iter=1, mode=cv2.GC_INIT_WITH_RECT, test=False):
    ...
    if preview>0:
        img0=img0.resize(preview)
        img0.ROI = QtCore.QRect(0, 0, img0.width, img0.height)
    ...
    mask = np.zeros(img0.cv2Img.shape[:2], dtype=np.uint8)
    mask_s = np.zeros(img0.cv2Img.shape[:2], dtype=np.uint8)
    ...
    tmp = np.zeros((img0.height, img0.width), dtype=np.uint8)
    if img0.rect is None:
        img0.rect = QRect(1000, 800, img0.width-1000, img0.height-800)
    tmp[img0.rect.top():img0.rect.bottom(), img0.rect.left():img0.rect.right()] = cv2.GC_PR_FGD
    tmp1=Qimage2array(img0.mask)
    #print Qimage2array(img0.mask)[:10,:10,]
    #exit()
    tmp1[tmp1==255]=cv2.GC_FGD
    tmp1[tmp1==0]=cv2.GC_BGD
    np.copyto(tmp, tmp1[:,:,1], where=(tmp1[:,:,0]>0))

    #mask = np.ascontiguousarray(Qimage2array(img0.mask)[:,:,3])
    mask=tmp
    mask_s= np.copy(mask)
    print mask.shape, img0.cv2Img.shape
    t0 = time.time()
    cv2.grabCut_slim(img0.cv2Img[:,:,:3],
                mask_s,
                QRect2tuple(img0.rect), bgdmodel, fgdmodel,
                nb_iter,
                mode)
    ...
    if True:
        ...
        fgdmodel = np.zeros((1, 13 * 5), np.float64)  # Temporary array for the foreground model
        t0 = time.time()
        cv2.grabCut(img0.cv2Img[:, :, :3],
                         mask,
                         QRect2tuple(img0.rect), bgdmodel, fgdmodel,
                         nb_iter,
                         mode)
        print 'grabcut_standard time :', time.time() - t0
    ...
    tmp = np.copy(img0.cv2Img)

    tmp[:, :, 3]=tmp[:,:,3]*mask2

    img1= MImage(cv2Img=tmp)
        #display
...
Mimg = MImage('F:\Bernard\epreuves\orch2-2-2.jpg')
